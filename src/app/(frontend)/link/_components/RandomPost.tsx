...
export function RandomPost() {
  ...
  const [postInfo, setPostInfo] = useState<RandomPostData | null>(null);
  ...
  const fetchRandomPost = useCallback(async () => {
    ...
    const newTip = RANDOM_POST_TIPS[Math.floor(Math.random() * RANDOM_POST_TIPS.length)];
    setTip(newTip);

    const isFirst = timesRef.current === 0;
    timesRef.current += 1;
    setTimes(timesRef.current);
    try {
      localStorage.setItem("randomPostTimes", String(timesRef.current));
    } catch {}

    const delay = isFirst ? 0 : Math.random() * 2000 + 1000;
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("randomPostTimes");
      if (saved) {
        const parsed = parseInt(saved, 10);
        setTimes(parsed);
        timesRef.current = parsed;
      }
    } catch {}
    fetchRandomPost();
    return () => {
  ...
  return (
    <div className="random-post-container">
      <div className="title-section">
        <div className="title-left">
          ...
          <a
            ...
            style={{ transform: `rotate(${360 * times}deg)`, transitionDuration: "0.3s" }}
      ...
      <div id="random-post" className="random-post-content">
        {loading ? (
          <div>{times >= 5 ? `钓鱼中... （Lv.${times} 当前称号：${honor}）` : "钓鱼中..."}</div>
        ) : postInfo ? (
          <div>
            {tip} 来自友链 <b>{postInfo.author}</b> 的文章：
            <a
              ...
              onClick={() => {
                try {
                  const clicks = parseInt(localStorage.getItem("randomPostClick") || "0", 10) + 1;
                  localStorage.setItem("randomPostClick", String(clicks));
                } catch {}
              }}
            >
              {postInfo.title}
